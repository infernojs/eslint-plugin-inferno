...
* `skipShapeProps`: In some cases it is impossible to accurately detect whether or not a `Inferno.PropTypes.shape`'s values are being used. Setting this option to `true` will skip validation of `PropTypes.shape`.
