...
// https://github.com/yannickcr/eslint-plugin-inferno/issues/7
...
module.exports = {
  meta: {
    docs: {
      description: 'Prevent missing props validation in a Inferno component definition',
  ...
  create: Components.detect(function(context, components, utils) {
    ...
    /**
     * Checks if prop should be validated by plugin-inferno-proptypes
    ...
    /**
     * Creates the representation of the Inferno propTypes for the component.
     * The representation is used to verify nested used properties.
     * @param {ASTNode} value Node of the Inferno.PropTypes for the desired property
     * @return {Object|Boolean} The representation of the declaration, true means
     *    the property is declared without the need for further analysis.
     */
    function buildInfernoDeclarationTypes(value) {
      if (
        value &&
        value.callee &&
        value.callee.object &&
      ...
      // Verify Inferno.PropTypes that are functions
      if (
        value &&
        ...
        switch (callName) {
          case 'shape':
            ...
            iterateProperties(argument.properties, function(childKey, childValue) {
              shapeTypeDefinition.children[childKey] = buildInfernoDeclarationTypes(childValue);
            });
          ...
          case 'objectOf':
            return {
              type: 'object',
              children: {
                __ANY_KEY__: buildInfernoDeclarationTypes(argument)
              }
            };
          case 'oneOfType':
            if (
            ...
            for (var i = 0, j = argument.elements.length; i < j; i++) {
              var type = buildInfernoDeclarationTypes(argument.elements[i]);
    ...
    /**
     * Creates the representation of the Inferno props type annotation for the component.
     * The representation is used to verify nested used properties.
      ...
      switch (propTypes && propTypes.type) {
        ...
        case 'ObjectExpression':
          iterateProperties(propTypes.properties, function(key, value) {
            ...
            declaredPropTypes[key] = buildInfernoDeclarationTypes(value);
          });
          break;
        case 'MemberExpression':
          ...
          if (propTypes && propTypes.parent && propTypes.property) {
            curDeclaredPropTypes[propTypes.property.name] =
              buildInfernoDeclarationTypes(propTypes.parent.right);
          } else {
            ignorePropsValidation = true;
          }
          break;
