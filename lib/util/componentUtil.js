...
function isExplicitComponent(node, context) {
  ...
  const relevantTags = commentAst.tags.filter((tag) => tag.name === 'Inferno.Component' || tag.name === 'Inferno.PureComponent');
...
/**
 * Checks if a component extends Inferno.PureComponent
 * @param {ASTNode} node
 * @param {Context} context
 * @returns {boolean}
 */
