...
describe('rule documentation files have the correct content', () => {
  const MESSAGES = {
    configs: '💼 This rule is enabled in the following [configs](https://github.com/infernojs/eslint-plugin-inferno#shareable-configurations):',
  ...
  function getConfigsForRule(ruleName, checkForEnabled) {
    ...
    Object.keys(plugin.configs).forEach((configName) => {
      const value = plugin.configs[configName].rules[`inferno/${ruleName}`];
  ...
  ruleFiles.forEach((ruleName) => {
    it(ruleName, () => {
      ...
      // Check title.
      const expectedTitle = `# ${rule.meta.docs.description} (inferno/${ruleName})`;
